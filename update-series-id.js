...
console.log('🔧 تنفيذ UPDATE tv_series SET id = tmdb_id\n');
console.log('═'.repeat(70));

async function main() {
  try {
    console.log('\n⚠️  هذا UPDATE على قاعدة الإنتاج (Turso)');
    console.log('📋 الأمر: UPDATE tv_series SET id = tmdb_id WHERE id != tmdb_id\n');
    
    // التحقق قبل التنفيذ
    const checkBefore = await turso.execute('SELECT COUNT(*) as count FROM tv_series WHERE id != tmdb_id');
    ...
    // التحقق بعد التنفيذ
    const checkAfter = await turso.execute('SELECT COUNT(*) as count FROM tv_series WHERE id != tmdb_id');
    ...
    if (countAfter === 0) {
      console.log('\n🎉 كل المسلسلات الآن لديها id = tmdb_id!');
    }
