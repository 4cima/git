// ============================================
// 🎬 سحب الأفلام الجديدة من آخر ID موجود
// ============================================
...
async function main() {
  ...
  const result = db.prepare('SELECT MAX(tmdb_id) as max_id FROM movies').get()
  ...
  for (let id = startId + 1; id <= endId; id++) {
    try {
      // تحقق إذا موجود
      const exists = db.prepare('SELECT tmdb_id FROM movies WHERE tmdb_id = ?').get(id)
      if (exists) {
        stats.exists++
        continue
      }
