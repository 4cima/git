// ============================================
// 📺 استيراد المسلسلات المفقودة من TMDB
// ============================================
const fs = require('fs')
...
async function main() {
  ...
  console.log('📺 استيراد المسلسلات المفقودة من TMDB')
  console.log('='.repeat(80))
  console.log('')
  
  const filePath = path.join(__dirname, '..', 'data', 'tv_series_ids_05_07_2026.json')
  ...
  const existing = new Set(db.prepare('SELECT tmdb_id FROM tv_series WHERE tmdb_id > 0').all().map(s => s.tmdb_id))
  ...
  const insertStmt = db.prepare(`
    INSERT INTO tv_series (tmdb_id, slug, title_ar, title_en, source, is_fetched, is_complete, created_at, updated_at)
    VALUES (?, ?, ?, ?, 'tmdb', 0, 0, datetime('now'), datetime('now'))
  `)
  ...
  for (let i = 0; i < lines.length; i++) {
    try {
      const series = JSON.parse(lines[i])
      
      if (existing.has(series.id)) {
        stats.skipped++
        continue
      }
      
      const title = series.original_name || `Series ${series.id}`
      const slug = `series-${series.id}-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-').substring(0, 50)}`
      
      batch.push({ id: series.id, slug, title_ar: title, title_en: title })
