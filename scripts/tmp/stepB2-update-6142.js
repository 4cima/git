...
for (const c of chunk(matched, 500)) {
  const grows = db.prepare("SELECT cg.content_tmdb_id AS id, g.slug AS slug FROM content_genres cg JOIN genres g ON g.tmdb_id=cg.genre_tmdb_id WHERE cg.content_type='movie' AND cg.content_tmdb_id IN (" + pq(c) + ")").all(...c);
  const byId = new Map();
...
console.log('wrote final + excluded lists');
...
const excl = [78632, 1533564, 1688163];
for (const id of excl) {
  const r = db.prepare('SELECT tmdb_id, vote_average, is_complete FROM movies WHERE tmdb_id=?').get(id);
  const flag = Number(r.is_complete) === 0 ? 'OK' : 'FAIL';
  console.log('excluded-vote ' + id + ' => ' + JSON.stringify(r) + ' ' + flag);
}
let seStill1 = 0;
...
console.log('LOCAL_VISITABLE before=' + lvBefore + ' after=' + lvAfter + ' delta=' + (lvAfter - lvBefore));
