...
/**
 * ============================================
 * 🔧 TV CAST REPAIR - TARGETED
 * ============================================
 * Purpose: Fetch missing cast for TV series
 * Does NOT touch translations or other data
 * ============================================
 */
 ...
 */
function insertCast(seriesId, cast) {
...
async function processSeries(series) {
  try {
    // Fetch cast from TMDB
    const cast = await fetchCastFromTMDB(series.tmdb_id);
    ...
    try {
      const inserted = insertCast(series.id, cast);
      
      // Update has_cast flag
...
async function main() {
  ...
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('🔧 إصلاح Cast للمسلسلات');
  console.log('═══════════════════════════════════════════════════════════════\n');
  ...
  console.log(`  نجح: ${stats.success.toLocaleString()} (${((stats.success/stats.total)*100).toFixed(1)}%)`);
  console.log(`  فشل: ${stats.failed.toLocaleString()} (${((stats.failed/stats.total)*100).toFixed(1)}%)`);
  console.log(`  تم تخطيه: ${stats.skipped.toLocaleString()} (${((stats.skipped/stats.total)*100).toFixed(1)}%)`);
