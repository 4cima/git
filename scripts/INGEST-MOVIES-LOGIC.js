// ============================================
// 🎬 MOVIES INGESTION LOGIC — v2 (SAFETY FIXED)
// ============================================
// التعديلات عن النسخة القديمة:
//  1) بيستخدم content-filter.js v2 (keywords + certifications + cast bug مُصلَّح)
//  2) content_rating بقى بيتخزن فعليًا في العمود age_rating (كان bug: كود ميت)
//  3) عند نجاح العنصر، is_filtered و filter_reason بيترجعوا لـ 0/NULL
//     (قبل كده لو عنصر اتفلتر غلط وبعدين اتصلح، كان فاضل معلّم للأبد)
// ============================================
...
async function processMovie(tmdbId) {
  try {
    ...
    // ملاحظة: النسخة القديمة كانت بتحسب المتغير ده لكن عمرها ما خزّنته
    // في الـ UPDATE (كود ميت). دلوقتي بيتخزن فعليًا في عمود age_rating.
    const content_rating = pickDisplayCertification(movie)
    ...
    if (e.message?.includes('404')) {
      ...
      stats.errors++
      if (process.env.DEBUG) console.error(`❌ فيلم ${tmdbId}: ${e.message}`)
    }
  }
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
async function main() {
  console.log('🎬 بدء تحديث بيانات الأفلام ⚡ (فلتر سلامة v2)\n')
