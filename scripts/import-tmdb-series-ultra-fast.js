// ============================================
// 📺 استيراد مسلسلات TMDB بأقصى سرعة
// ============================================
const fs = require('fs')
...
async function main() {
  ...
  console.log('📺 استيراد مسلسلات TMDB بأقصى سرعة')
  console.log('='.repeat(80))
  console.log('')
  
  const filePath = path.join(__dirname, '..', 'data', 'tv_series_ids_07_16_2026.json')
  ...
  const insertStmt = db.prepare(`
    INSERT OR IGNORE INTO tv_series (tmdb_id, title_en, is_fetched, is_complete, created_at, updated_at)
  ...
  for (let i = 0; i < lines.length; i++) {
    try {
      ...
      batch.push({ id: series.id, title: series.original_name || `Series ${series.id}` })
