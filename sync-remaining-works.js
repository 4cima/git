...
async function syncMovies() {
  console.log('🎬 مزامنة الأفلام المتبقية\n');
  ...
  console.log(`📦 عدد الأفلام المتبقية: ${movies.length.toLocaleString()}`);
  console.log(`📦 عدد الدفعات: ${Math.ceil(movies.length / BATCH_SIZE)}\n`);
  ...
  for (let i = 0; i < movies.length; i += BATCH_SIZE) {
    ...
    for (const item of batch) {
      try {
        const m = db.prepare('SELECT * FROM movies WHERE id = ?').get(item.id);
        ...
        db.prepare('UPDATE movies SET synced_to_turso = 1 WHERE id = ?').run(item.id);
        synced++;
      } catch (error) {
        errors++;
        console.error(`❌ خطأ في فيلم ${item.id}: ${error.message}`);
      }
    }
    
    const elapsed = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
    const rate = synced > 0 ? (synced / elapsed * 60).toFixed(0) : 0;
    const percent = ((synced / movies.length) * 100).toFixed(1);
    const eta = synced > 0 ? ((movies.length - synced) / (synced / elapsed) / 60).toFixed(0) : '?';
    
    process.stdout.write(`\r⏳ ${synced.toLocaleString()} / ${movies.length.toLocaleString()} (${percent}%) | ${rate}/دقيقة | ETA: ${eta} دقيقة`);
  }
  
  const totalTime = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
  
  console.log('\n\n✅ اكتملت مزامنة الأفلام!');
  console.log(`📊 الأفلام: ${synced.toLocaleString()}`);
  console.log(`❌ الأخطاء: ${errors.toLocaleString()}`);
  console.log(`⏱️  الوقت: ${totalTime} دقيقة`);
  console.log(`⚡ السرعة: ${(synced / totalTime).toFixed(0)} فيلم/دقيقة\n`);
}

async function syncSeries() {
  console.log('📺 مزامنة المسلسلات المتبقية\n');
  ...
  console.log(`📦 عدد المسلسلات المتبقية: ${series.length.toLocaleString()}`);
  ...
  for (let i = 0; i < series.length; i += BATCH_SIZE) {
    ...
    for (const item of batch) {
      try {
        ...
        errors++;
        console.error(`❌ خطأ في مسلسل ${item.id}: ${error.message}`);
      }
    }
    ...
    const eta = synced > 0 ? ((series.length - synced) / (synced / elapsed) / 60).toFixed(0) : '?';
...
async function main() {
  console.log('🔄 بدء مزامنة الأعمال المتبقية\n');
