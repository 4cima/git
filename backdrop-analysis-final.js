...
async function analyzeBackdropMissing() {
  ...
  console.log('\n' + '='.repeat(60));
  console.log('الخلاصة:');
  const mPct = ((hasInLocal / moviesNoBd.rows.length) * 100).toFixed(1);
  const sPct = ((sHasInLocal / seriesNoBd.rows.length) * 100).toFixed(1);
  console.log('الأفلام:', mPct + '% عندهم backdrop في Local لكن فشل UPDATE');
  console.log('المسلسلات:', sPct + '% عندهم backdrop في Local لكن فشل UPDATE');
  console.log('='.repeat(60));
  
  d.close();
}

analyzeBackdropMissing().catch(console.error);
