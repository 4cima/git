...
import { Tv, Plus, Search, Edit, Trash2, Loader, X, Save, Layers, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react'
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">إجمالي {totalCount} مسلسل • صفحة {page} من {totalPages}</p>
        </div>
        <Link href="/admin/add-series" className="bg-purple-500 hover:bg-purple-400 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors">
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3">
          ...
          <div className="relative">
            <button onClick={() => setOpenDropdown(openDropdown === 'genre' ? null : 'genre')}
              className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:border-purple-500 outline-none flex items-center gap-2 min-w-[140px] justify-between">
            ...
            {openDropdown === 'genre' && (
              <div className="absolute top-full left-0 mt-1 bg-zinc-950 border border-zinc-800 rounded-lg shadow-2xl z-50 min-w-full max-h-[300px] overflow-y-auto">
                <button onClick={() => { handleFilterChange(() => setSelectedGenre('all')); setOpenDropdown(null) }}
                  className={`w-full text-right px-3 py-2 text-sm hover:bg-zinc-800 ${selectedGenre === 'all' ? 'bg-zinc-800 text-purple-400' : 'text-zinc-100'}`}>
                  كل التصنيفات
                </button>
                {GENRES.map(g => (
                  <button key={g.name} onClick={() => { handleFilterChange(() => setSelectedGenre(g.name)); setOpenDropdown(null) }}
                    className={`w-full text-right px-3 py-2 text-sm hover:bg-zinc-800 whitespace-nowrap ${selectedGenre === g.name ? 'bg-zinc-800 text-purple-400' : 'text-zinc-100'}`}>
                    {g.name}
                  </button>
                ))}
              </div>
          ...
          <div className="relative">
            <button onClick={() => setOpenDropdown(openDropdown === 'year' ? null : 'year')}
              className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:border-purple-500 outline-none flex items-center gap-2 min-w-[120px] justify-between">
              ...
              <div className="absolute top-full left-0 mt-1 bg-zinc-950 border border-zinc-800 rounded-lg shadow-2xl z-50 min-w-full max-h-[300px] overflow-y-auto">
                {YEARS.map(y => (
                  <button key={y.value} onClick={() => { handleFilterChange(() => setSelectedYear(y.value)); setOpenDropdown(null) }}
                    className={`w-full text-right px-3 py-2 text-sm hover:bg-zinc-800 ${selectedYear === y.value ? 'bg-zinc-800 text-purple-400' : 'text-zinc-100'}`}>
                    {y.label}
                  </button>
                ))}
              </div>
              ...
              <div className="absolute top-full left-0 mt-1 bg-zinc-950 border border-zinc-800 rounded-lg shadow-2xl z-50 min-w-full max-h-[300px] overflow-y-auto">
                {RATINGS.map(r => (
                  <button key={r.value} onClick={() => { handleFilterChange(() => setSelectedRating(r.value)); setOpenDropdown(null) }}
                    className={`w-full text-right px-3 py-2 text-sm hover:bg-zinc-800 ${selectedRating === r.value ? 'bg-zinc-800 text-purple-400' : 'text-zinc-100'}`}>
                    {r.label}
                  </button>
                ))}
              </div>
          ...
          <div className="relative">
            <button onClick={() => setOpenDropdown(openDropdown === 'sort' ? null : 'sort')}
              className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:border-purple-500 outline-none flex items-center gap-2 min-w-[130px] justify-between">
              ...
              <div className="absolute top-full left-0 mt-1 bg-zinc-950 border border-zinc-800 rounded-lg shadow-2xl z-50 min-w-full max-h-[300px] overflow-y-auto">
                {SORT_OPTIONS.map((o, idx) => (
                  <button key={idx} onClick={() => { handleFilterChange(() => { setSortBy(o.value); setSortOrder(o.order) }); setOpenDropdown(null) }}
                    className={`w-full text-right px-3 py-2 text-sm hover:bg-zinc-800 whitespace-nowrap ${sortBy === o.value && sortOrder === o.order ? 'bg-zinc-800 text-purple-400' : 'text-zinc-100'}`}>
                    {o.label}
                  </button>
                ))}
              </div>
      ...
      {loading ? (
        ...
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl p-12 text-center">
          ...
          <p className="text-zinc-400">لا توجد مسلسلات مطابقة</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-4">
          {series.map((item) => {
            ...
            return (
              <div key={item.id} className="group relative bg-zinc-900/20 border border-zinc-800/60 hover:border-purple-500/50 rounded-xl overflow-hidden transition-all duration-300">
                <div className="aspect-[2/3] w-full relative overflow-hidden bg-zinc-950">
                  {item.poster_path ? (
                    <img src={`/tmdb/w185${item.poster_path}`} alt={item.name_ar || item.name_en || ''}
                  ...
                  {item.vote_average && item.vote_average > 0 && (
                    <div className="absolute top-2 left-2 z-20">
                      <span className={`px-2 py-1 rounded-lg text-[9px] font-bold backdrop-blur-md shadow-lg ${
                        item.vote_average >= 7 ? 'bg-green-600/90 text-white' : 'bg-yellow-600/90 text-white'
                      }`}>
                  ...
                  {/* Edit/Delete/Seasons overlay */}
                  <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2 z-30">
                    <button onClick={() => router.push(`/admin/series/${item.tmdb_id}`)}
                      className="p-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-lg transition-colors" title="إدارة المواسم">
                      <Layers size={16} />
                    </button>
                    <button onClick={() => handleEdit(item)}
                      className="p-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors" title="تعديل">
                      <Edit size={16} />
                    </button>
                    <button onClick={() => handleDelete(item)} disabled={deleting === item.tmdb_id}
                      className="p-2.5 bg-red-600 hover:bg-red-500 text-white rounded-lg transition-colors disabled:opacity-50" title="حذف">
                      {deleting === item.tmdb_id ? <Loader size={16} className="animate-spin" /> : <Trash2 size={16} />}
                    </button>
                  </div>
                </div>
                
                <div className="p-2 h-[48px] flex flex-col justify-center">
                  <h3 className="text-[11px] font-bold text-zinc-200 line-clamp-1 leading-tight">{item.name_ar || item.name_en}</h3>
                  {item.name_en && item.name_ar && (
                    <p className="text-[9px] text-zinc-400 line-clamp-1 mt-0.5 leading-tight">{item.name_en}</p>
                  )}
                </div>
              </div>
            )
      ...
      {!loading && totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-sm text-zinc-400">
            صفحة {page} من {totalPages} • {totalCount} مسلسل
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="number" min="1" max={totalPages} value={jumpToPage}
              ...
              className="w-16 bg-zinc-950 border border-zinc-800 rounded-lg px-2 py-1.5 text-sm text-zinc-100 text-center focus:border-purple-500 outline-none" />
            <button onClick={handlePageJump} disabled={!jumpToPage}
              className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
              اذهب
            </button>
          </div>
        </div>
