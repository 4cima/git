...
import { Search, Tv, Plus, Save, Loader, X, Calendar, Star } from 'lucide-react'
...
export default function AddSeriesPage() {
  ...
  const [selected, setSelected] = useState<TMDBSeries | null>(null)
  ...
  const searchSeries = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/tmdb/search?type=tv&query=${encodeURIComponent(query)}`)
  ...
  const saveSeries = async () => {
    ...
    try {
      ...
      flash('ok', `تم إضافة "${selected.name}" بنجاح`)
      setSelected(null)
    } catch (e) {
    ...
    <div className="space-y-6" dir="rtl">
      <div>
        <h1 className="text-2xl font-bold text-zinc-100 flex items-center gap-2">
          <Tv className="text-purple-400" /> إضافة مسلسل جديد
        </h1>
        <p className="text-sm text-zinc-400 mt-1">ابحث في قاعدة TMDB واختر المسلسل لإضافته</p>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        <form onSubmit={searchSeries} className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-700 rounded-lg py-3 pr-12 pl-4 text-zinc-100 text-sm focus:outline-none focus:border-purple-500 transition-colors"
              dir="rtl"
            />
          </div>
          <button
            ...
            className="bg-purple-500 hover:bg-purple-400 text-white font-bold px-6 rounded-lg transition-colors flex items-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mt-6">
            {results.map((series) => (
              <button
                ...
                className="group relative aspect-[2/3] rounded-lg overflow-hidden hover:ring-2 ring-purple-500 transition-all"
              >
                {series.poster_path ? (
                  <img
                  ...
                  <div className="w-full h-full bg-zinc-800 flex items-center justify-center">
                    <Tv className="text-zinc-600" size={32} />
                  </div>
                )}
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="font-bold text-sm text-white line-clamp-2">{series.name}</p>
                  <p className="text-xs text-zinc-400">{series.first_air_date?.split('-')[0]}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="grid md:grid-cols-[250px_1fr] gap-6 bg-zinc-900 border border-zinc-800 p-6 rounded-xl">
          ...
          <div className="space-y-3">
            <div className="aspect-[2/3] rounded-xl overflow-hidden border border-zinc-700 shadow-2xl">
              {selected.poster_path ? (
                ...
                <div className="w-full h-full bg-zinc-800 flex items-center justify-center">
                  <Tv className="text-zinc-600" size={48} />
                </div>
              )}
            </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              {selected.original_name !== selected.name && (
                <p className="text-sm text-zinc-500">{selected.original_name}</p>
              )}
              <p className="text-zinc-400 leading-relaxed text-sm">{selected.overview}</p>
              <div className="flex gap-3 text-xs pt-2">
                <span className="flex items-center gap-1 bg-purple-500/10 px-2 py-1 rounded border border-purple-500/20 text-purple-400">
                  <Calendar size={12} /> {selected.first_air_date}
                </span>
            ...
            <div className="pt-4 flex gap-3">
              <button
                onClick={saveSeries}
                disabled={saving}
                className="flex-1 bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
              >
                {saving ? (
                  <>
