/**
 * /api/admin/movies — إدارة الأفلام من اللوحة.
 * GET: قائمة بلا بوابة filter_status (الأدمن يشوف كل الحالات) — ترقيم LIMIT/OFFSET
 *      بحدّ page ≤ 100 و limit ≤ 60 ⇒ أقصى قراءة ~6K صف (استخدام إداري نادر).
 * PATCH/DELETE: نفس عقود القديم + requireAdmin إلزامي + مسح كاش الحافة بعد التغيير
 *      حتى يظهر الأثر على صفحات الزوار (الكاش 30 يومًا).
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (search) {
      where.push('(title_ar LIKE ? OR title_en LIKE ? OR CAST(tmdb_id AS TEXT) = ?)')
    ...
    const sortCol = ['updated_at', 'release_year', 'vote_average', 'popularity', 'title_ar'].includes(sort) ? sort : 'updated_at'
    const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : ''

    const rows = await executeAll(
      `SELECT tmdb_id, slug, title_ar, title_en, poster_path, release_year, vote_average, filter_status, updated_at
    ...
    return NextResponse.json({ ok: true, movies: rows, pagination: { page, limit, hasMore } }, { headers: NO_STORE })
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  ...
  // مسح كاش الحافة best-effort — التعديل يظهر على الموقع فورًا بدل انتظار انتهاء الكاش
...
export async function DELETE(request: NextRequest) {
  ...
  const check = await executeFirst('SELECT tmdb_id, title_en, title_ar FROM movies WHERE tmdb_id = ?', [tmdb_id])
  if (!check) return NextResponse.json({ ok: false, error: `tmdb_id=${tmdb_id} not found` }, { status: 404 })
  ...
  return NextResponse.json({ ok: true, tmdb_id, deleted_title: check.title_ar || check.title_en, cache_purged: purge.ok })
}
