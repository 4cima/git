...
const COLOR_MAP: Record<SectionColor, { text: string; underline: string; ring: string }> = {
  cyan: { text: 'text-cyan-400', underline: 'from-cyan-400', ring: 'group-hover:ring-cyan-400/60' },
  purple: { text: 'text-purple-400', underline: 'from-purple-400', ring: 'group-hover:ring-purple-400/60' },
  gold: { text: 'text-yellow-400', underline: 'from-yellow-400', ring: 'group-hover:ring-yellow-400/60' },
  green: { text: 'text-green-400', underline: 'from-green-400', ring: 'group-hover:ring-green-400/60' },
}

export function DynamicSection({ items, title, subtitle, color, link }: DynamicSectionProps) {
  const { ref: scrollerRef, isDragging, handleMouseDown, consumeIfDragged } = useDragScroll<HTMLDivElement>()
  ...
  return (
    <div dir="rtl" className="mx-auto max-w-7xl px-4 sm:px-6">
      <div className="mb-6 flex items-end justify-between">
        <div className="text-right">
          <h2 className={`text-xl font-extrabold sm:text-2xl ${palette.text}`}>{title}</h2>
          <div className={`mt-2 mb-1 h-1 w-16 rounded-full bg-gradient-to-l ${palette.underline} to-transparent`} />
          <p className="text-sm text-gray-400">{subtitle}</p>
        </div>

        <Link href={link} className="flex shrink-0 items-center gap-1 text-sm font-semibold text-gray-300 transition hover:text-white">
          عرض الكل
      ...
      <div className="group/scroller relative">
        ...
        >
          {list.map((item) => (
            <Link key={item.id} href={itemHref(item)} onClick={handleCardClick} className="group w-40 shrink-0 md:w-48" draggable={false}>
              <div className={`relative aspect-[2/3] overflow-hidden rounded-lg ring-1 ring-white/10 transition duration-300 group-hover:scale-[1.03] group-hover:shadow-xl group-hover:shadow-black/50 ${palette.ring}`}>
                <img src={getCardImageUrl(item)} alt={item.title} className="h-full w-full object-cover" loading="lazy" decoding="async" draggable={false} />

                <div className="absolute inset-x-1.5 top-1.5 flex items-start justify-start gap-1">
                  {item.primary_genre && <span className="rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-semibold text-gray-200 backdrop-blur-sm">{item.primary_genre}</span>}
                ...
                <div className="absolute inset-x-1.5 bottom-1.5 text-right">
                  <span className="rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-semibold text-gray-300 backdrop-blur-sm">{item.year}</span>
                </div>
                ...
                {item.overview_ar && (
                  <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-gray-950 via-gray-950/85 to-transparent p-2.5 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <p className="line-clamp-6 text-right text-[11px] leading-snug text-gray-200">{item.overview_ar}</p>
                  </div>
                )}
              </div>
