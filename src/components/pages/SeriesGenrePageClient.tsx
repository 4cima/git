...
import { YEARS, RATINGS, COUNTRIES, LANGUAGES, SERIES_SORT_OPTIONS as SORT_OPTIONS,
...
interface SeriesGenrePageClientProps {
  ...
  /** رابط صفحة نظير الأفلام (افتراضي: /movies/genres/{slug}) */
  moviesHref?: string
  /** ترقيم ساكن من السيرفر (روابط <a> للزحف) — غيابه = سكرول لانهائي فقط */
  staticPagination?: StaticPagination | null
}

export function SeriesGenrePageClient({ genre, slug, initialSeries, initialHasMore, moviesHref, staticPagination }: SeriesGenrePageClientProps) {
  ...
  /* (E-11) فرع slug === 'arabic' حُذف: لا يوجد صف 'arabic' في جدول genres
     (تحقق مباشر من قاعدة البيانات)، والقسم العربي مساره الرسمي /series/lang/ar
  ...
  const headerActions = (
    <>
      ...
      <HeaderLinkButton href={moviesHref ?? `/movies/genres/${slug}`} accent="movie" chevron>أفلام {genre.name_ar}</HeaderLinkButton>
    </>
  )
  ...
  // Fetch series — نفس منطق صفحات اللغة: /api/series والتصنيف مثبّت كفلتر + الفلاتر الكاملة
  useEffect(() => {
  ...
  }, [slug, sort, order, page, initialSeries.length, retryNonce, selectedYear, selectedRating, selectedCountry, selectedLanguage, debouncedSearch])
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-12">
      <div className="page-container">
        ...
        <ListingPageHeader
          variant="series"
          title={`مسلسلات ${genre.name_ar}`}
          description={`استكشف جميع مسلسلات ${genre.name_ar} المترجمة`}
        ...
        <div ref={filtersRef}>
          <CinematicFilterBar accent="series">
            ...
            <div className="flex flex-wrap items-center gap-3 order-2 md:order-1">
              ...
              />
              {/* (E-10) فلتر اللغة — نفس CinematicDropdown الموحّد، ويُرسل original_language إلى /api/series */}
              <CinematicDropdown
                  ...
                  <Fragment key={`srest-${ci}`}>
                    <div className="grid-responsive gap-4" suppressHydrationWarning>
                      {restItems.slice(ci * 8, ci * 8 + 8).map((item: any, i: number) => {
                        const enhancedItem = { ...item, media_type: 'tv', isSeries: true }
                        return (
                          <MovieCard movie={enhancedItem} index={LISTING_TOP_CARDS_COUNT + ci * 8 + i} forceTv />
                        )
                      })}
                    </div>
