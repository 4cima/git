...
export function MoviesPageClient({ initialMovies = [], initialHasMore = false }: { initialMovies?: any[]; initialHasMore?: boolean }) {
  const { user } = useAuth() // Check if user is logged in
  const searchParams = useSearchParams()
  /* قائمة تصنيفات ديناميكية من قاعدة البيانات (GENRES احتياطية حتى وصول الاستجابة) */
  const genresList = useListingGenres('movie', GENRES)
  ...
  /* تحديث بدون قفز: عند تغيير فلتر والمحتوى معروض، يبقى مكانه ونعرض شريط تقدم رفيع
     بدل إسقاط الشبكة كلها وسكبور (skeleton) → يمنع انهيار الصفحة وقفز أسفلها */
  ...
  useEffect(() => {
    ...
    /* تخطي الجلب الأول: بيانات الـSSR جاهزة ولا توجد فلاتر نشطة أو إعادة محاولة
       (نفس منطق صفحات التصنيفات — يوفر طلبًا كاملًا ويمنع وميض القائمة) */
    if (
      retryNonce === 0 &&
      page === 1 &&
      sortBy === 'popularity' &&
    ...
    if (isFirstPage) {
      ...
      if (movies.length > 0) setRefreshing(true)
  ...
  useEffect(() => {
    ...
    const missing = movies
      .map(m => ({ content_type: 'movie', tmdb_id: (m.tmdb_id || m.id) as number | string }))
      .filter(i => !fetchedStateKeys.current.has(`movie-${i.tmdb_id}`))
    if (missing.length === 0) return
    missing.forEach(i => fetchedStateKeys.current.add(`movie-${i.tmdb_id}`))
